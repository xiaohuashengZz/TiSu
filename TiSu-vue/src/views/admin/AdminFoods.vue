  ...
  <div class="admin-foods">
    ...
    <el-card shadow="never" style="margin-bottom: var(--space-md)">
      <div class="filter-bar">
        <el-radio-group v-model="sourceFilter" size="small">
          ...
          <el-radio-button value="user">用户食物</el-radio-button>
        </el-radio-group>
    ...
    <el-card shadow="never">
      <el-table :data="foods" stripe style="width: 100%">
        ...
        <el-table-column prop="fiber" label="纤维(g)" width="90" />
...
const fetchFoods = async () => {
  try {
    ...
    const res = await getAdminFoods(params)
...
watch([sourceFilter, categoryFilter, keyword, pageSize], () => {
  currentPage.value = 1
  fetchFoods()
})
...
const handleSave = async () => {
  ...
  try {
    if (isEditing.value) {
      await updateAdminFood(editId, { ...form })
      ElMessage.success('已保存')
    } else {
      await createAdminFood({ ...form, source: 'official' })
